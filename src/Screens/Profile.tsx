...
const Profile = ({
  ...
  const handleEdit = async (attribute: string, value: string) => {
    ...
    const route = `http://${process.env.EXPO_PUBLIC_API_URL}:5000/user_info/${username}/${attribute}/${value}`
  ...
  const deleteAccount = async () => {
    const route = `http://${process.env.EXPO_PUBLIC_API_URL}:5000/user_info/${username}/delete`
    const response = await fetch(route)
    const data = await response.json()
    console.log(data)
